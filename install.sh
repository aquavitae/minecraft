...
for path in addons/*; do
  ...
  rclone copy --retries=1 ./addons/$f/behavior :ftp:development_behavior_packs/$f || echo "ok"
  rclone mkdir :ftp:development_resource_packs/$f
  rclone copy --retries=1 ./addons/$f/resource :ftp:development_resource_packs/$f || echo "ok"
done

rclone copy --retries=1 world :ftp:worlds/Legoland || echo "ok"
rclone copy --retries=1 settings :ftp: || echo "ok"
